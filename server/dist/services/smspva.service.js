...
// Mapping from SMS-Activate country codes to SMSPVA country codes
const REVERSE_COUNTRY_CODE_MAPPING = {
    'RU': '0',
    'UA': '1',
    'KZ': '2',
    'CN': '3',
...
exports.smspvaService = {
    ...
    async getRentServicesAndCountries() {
        ...
        try {
            ...
            const countriesResponse = await (0, retry_1.retryOperation)(() => axios_1.default.get(API_BASE_URL, {
                params: {
                    method: 'getcountries',
                    apikey: API_KEY
                },
            ...
            for (const country of testCountries) {
                try {
                    ...
                    const servicesResponse = await (0, retry_1.retryOperation)(() => axios_1.default.get(API_BASE_URL, {
                        params: {
                            method: 'getdata',
                            country: country,
                            apikey: API_KEY
                    ...
                    if (servicesResponse.data?.status === 1 && servicesResponse.data.data?.services) {
                        ...
                        countryServices.forEach((service) => {
                            const existingService = allServicesData.services.find((s) => s.service === service.service);
                            if (!existingService) {
     ...
     */
    async getRentNumber(service, rentTime = '4', operator = 'any', country = 'RU') {
        try {
                    ...
                    if (errorMsg.includes('No numbers available') ||
                        ...
                        try {
                            ...
                            if (!availabilityCheck.available) {
                                throw new SmspvaError(`No phone numbers available for service ${smspvaService} in country ${smspvaCountry}. Available count: ${availabilityCheck.count}`, 'NO_NUMBERS_AVAILABLE');
                            }
                        }
                        catch (checkError) {
            ...
            throw new SmspvaError(error.message || 'Failed to rent SMSPVA number', 'UNKNOWN_ERROR');
        }
    },
    /**
     * Get rental status and messages using SMSPVA Rental API
     */
    async getRentStatus(id, page = '0', size = '50', service = 'opt1', country = 'RU') {
        try {
     ...
     */
    async getRentalMessages(id, country = 'RU', service = 'opt1') {
        try {
     ...
     */
    async checkServiceAvailability(country, service) {
        try {
            const response = await (0, retry_1.retryOperation)(() => axios_1.default.get(API_BASE_URL, {
                params: {
                    method: 'getdata',
    ...
    transformServicesResponse(data) {
        try {
            ...
            if (servicesArray) {
                ...
                Object.entries(data.services).forEach(([smspvaCode, serviceData]) => {
                    services.push({
    ...
    transformRentalMessages(data) {
        ...
        if (data && data.status === 1 && data.data) {
            ...
            const messages = messageList.map((sms, index) => {
    ...
    transformRentListResponse(data) {
        ...
        if (data && data.status === 1 && data.data && Array.isArray(data.data)) {
            const rentals = data.data.map((rental) => ({
