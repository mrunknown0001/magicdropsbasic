...
exports.sendAutoReply = exports.getConversations = exports.uploadAttachment = exports.sendMessage = exports.getMessages = exports.createConversation = void 0;
...
const createConversation = async (req, res) => {
    try {
        ...
        const { data: existingConversation, error: existingError } = await supabase_1.supabase
...
const sendMessage = async (req, res) => {
    try {
        ...
        // Store user message
        const { data: userMessage, error: messageError } = await supabase_1.supabase
            .from('chat_messages')
            .insert({
        ...
        const { getManagerStatus, generateAwayMessage } = await Promise.resolve().then(() => __importStar(require('../utils/humanBehavior')));
        ...
        if (!managerStatus.isAvailable) {
            ...
            if (awayMessage) {
                ...
                const { data: aiMessage, error: aiMessageError } = await supabase_1.supabase
                    .from('chat_messages')
                    .insert({
                    conversation_id: conversationId,
                    ...
                    metadata: {
                        ...
                        is_auto_reply: true,
                        promised_return_time: scheduler_service_1.SchedulerService.calculateReturnTime(managerStatus).toISOString()
                    }
                })
                    .select()
                    .single();
                ...
                await followUp_service_1.FollowUpService.createFollowUp({
                    conversationId,
                    userId,
                    originalMessageId: userMessage.id,
                    autoReplyMessageId: aiMessage.id,
                    promisedReturnTime: scheduler_service_1.SchedulerService.calculateReturnTime(managerStatus),
                    originalUserMessage: content.trim(),
                    urgencyLevel
                });
                console.log(`📅 Auto-reply sent and follow-up scheduled for ${scheduler_service_1.SchedulerService.calculateReturnTime(managerStatus).toLocaleString('de-DE')}`);
                return res.json({
                    success: true,
                    userMessage: {
                        id: userMessage.id,
        ...
        if (knowledgeArticles.length === 0) {
            const { data: fallbackArticles } = await supabase_1.supabase
        ...
        const { data: recentMessages } = await supabase_1.supabase
            .from('chat_messages')
            .select(`
        sender_id, 
        content, 
      ...
      `)
        ...
        const openAIMessages = [
            ...
            ...(recentMessages || [])
                ...
                if (hasImageAttachment) {
                    ...
                    const imageUrl = supabase_1.supabase.storage
        ...
        const hasImages = openAIMessages.some(msg => Array.isArray(msg.content) &&
            msg.content.some((item) => item.type === 'image_url'));
        // Get AI response (use vision-capable model if images are present)
        const startTime = Date.now();
        const aiResponse = await openai_service_1.OpenAIService.createChatCompletion(openAIMessages, {
        ...
        if (knowledgeArticles && knowledgeArticles.length > 0) {
            try {
                ...
                for (const article of knowledgeArticles) {
                    ...
                    const { data: currentArticle } = await supabase_1.supabase
                        .from('knowledge_base_articles')
...
const uploadAttachment = async (req, res) => {
    try {
        ...
        const { data: attachment, error: attachmentError } = await supabase_1.supabase
 ...
 */
async function buildWorkerContext(userId, taskAssignmentId) {
    try {
        // Get worker profile
        ...
        const { data: chatManager } = await supabase_1.supabase
        ...
        const { data: taskAssignments } = await supabase_1.supabase
...
- Projektname: ${currentTask.task_template?.title || 'Unbekannt'}
...
- Geplante Arbeitszeit: ${currentTask.task_template?.estimated_hours || 'Wird noch festgelegt'} Stunden
- Projektvergütung: €${currentTask.custom_payment_amount || currentTask.task_template?.payment_amount || 'Wird noch festgelegt'}`;
            }
        }
        ...
        if (taskAssignments && taskAssignments.length > 0) {
            ...
            taskAssignments.forEach((assignment, index) => {
                const title = assignment.task_template?.title || 'Unbenanntes Projekt';
        ...
        if (knowledgeArticles && knowledgeArticles.length > 0) {
            ...
            knowledgeArticles.forEach(article => {
                systemPrompt += `\n\n**${article.title}** (${article.knowledge_base_categories?.name || 'Uncategorized'})
 ...
 */
async function performInternalSemanticSearch(query, limit = 5, threshold = 0.6) {
    try {
      ...
      `)
        ...
        const similarities = articlesWithEmbeddings.map(item => {
            const articleEmbedding = item.embedding;
 ...
 */
function detectUrgencyFromMessage(message) {
...
const sendAutoReply = async (req, res) => {
    try {
        ...
        const { conversationId, userMessage, awayMessage, managerStatus, urgencyLevel } = req.body;
        if (!conversationId || !userMessage || !awayMessage) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        // Store user message first
        const { data: userMessageData, error: userMessageError } = await supabase_1.supabase
            .from('chat_messages')
            .insert({
            conversation_id: conversationId,
            sender_id: userId,
        ...
        // Store auto-reply message
        const { data: autoReplyData, error: autoReplyError } = await supabase_1.supabase
            .from('chat_messages')
            .insert({
            conversation_id: conversationId,
            sender_id: null, // AI assistant
        ...
        const followUp = await followUp_service_1.FollowUpService.createFollowUp({
            conversationId,
            userId,
        ...
        res.json({
            ...
            followUpTime: followUp ? scheduler_service_1.SchedulerService.calculateReturnTime(managerStatus).toISOString() : null
        });
    }
    catch (error) {
