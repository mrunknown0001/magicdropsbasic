...
// === GOGETSMS API INTERFACES ===

// Dual-mode operation types
export type GoGetSmsMode = 'activation' | 'rental';

// === ACTIVATION MODE INTERFACES ===
export interface ActivationServiceInfo {
  code: string;
  name: string;
  cost: number;
...
export interface ActivationStatusResponse {
  status: 'STATUS_WAIT_CODE' | 'STATUS_CANCEL' | 'STATUS_OK';
  code?: string; // SMS code if received
}

export interface ActivationServicesResponse {
  services: Record<string, ActivationServiceInfo>;
  countries: Record<string, string>; // Numeric codes: "16" -> "GB"
}

// === RENTAL MODE INTERFACES ===
export interface RentalServiceInfo {
  code: string;
  name: string;
  cost: number;
  available: boolean;
...
export interface RentalServicesResponse {
  services?: Record<string, RentalServiceInfo>;
  countries: Record<string, string>; // 2-letter codes: "GB", "DE", "US"
}

// === UNIFIED INTERFACES ===
export interface GoGetSmsServiceResponse {
  mode: GoGetSmsMode;
  activation?: ActivationServicesResponse;
  rental?: RentalServicesResponse;
}

export interface GoGetSmsNumberResponse {
  mode: GoGetSmsMode;
  success: boolean;
  data: ActivationResponse | RentalResponse;
  cost: number;
  phoneNumber: string;
  id: string;
  endDate?: string;
...
class GoGetSmsRateLimit {
  ...
  private readonly TIME_WINDOW = 60000; // 1 minute in milliseconds
...
const FULL_COUNTRY_RENTAL_MAPPING: Record<string, string> = {
...
const ACTIVATION_COUNTRY_MAPPING: Record<string, string> = {
...
const RENTAL_COUNTRY_MAPPING: Record<string, string> = {
...
const COUNTRY_CODE_MAPPING: Record<string, { activation: string, rental: string }> = {
...
export const gogetSmsService = {
   ...
   */
  handleApiError(error: any): GoGetSmsError {
   ...
   */
  async makeApiRequest(params: Record<string, any>) {
    ...
    if (typeof response.data === 'string') {
      ...
      if (['BAD_SERVICE', 'BAD_COUNTRY', 'BAD_KEY', 'BAD_ACTION'].includes(errorCode)) {
        ...
        (fakeError as any).response = { data: response.data };
        throw fakeError;
      }
    }
   ...
   */
  async getPrices(country: string = '0') {
    try {
   ...
   */
  async getNumberV2(service: string, country: string = '0') {
    try {
  ...
  async getRentNumber(
    ...
    let gogetSmsService: string = '1'; // Default to OTHER
    let gogetSmsCountry: string = 'GB'; // Default to UK
  ...
  country: '${gogetSmsCountry}'
      ...
      if (error.response?.data === 'BAD_COUNTRY') {
          ...
          for (const fallbackCountry of fallbackCountries) {
            if (fallbackCountry === gogetSmsCountry) continue; // Skip the one we already tried
   ...
   */
  async getRentStatus(id: string, page: string = '0', size: string = '10') {
    try {
   ...
   */
  async setRentStatus(id: string, status: string) {
    try {
   ...
   * Extend a rental
   */
  async continueRentNumber(id: string, rentTime: string) {
    try {
   ...
   */
  async getActivationServices(country?: string): Promise<ActivationServicesResponse> {
    try {
   ...
   */
  async getActivationStatus(activationId: string): Promise<ActivationStatusResponse> {
    try {
  ...
  async getRentalServices(
    rentTime: string = '4',
    country?: string
  ): Promise<RentalServicesResponse> {
   ...
   */
  async getDualModeServices(
    mode: GoGetSmsMode = 'rental',
    rentTime?: string,
    country?: string
  ): Promise<GoGetSmsServiceResponse> {
    try {
  ...
  async getPhoneNumber(
    ...
    country: string,
    rentTime?: string
  ): Promise<GoGetSmsNumberResponse> {
    try {
   ...
   */
  detectOptimalMode(service: string, duration?: string): GoGetSmsMode {
   ...
   */
  transformActivationServicesResponse(data: any, country: string): ActivationServicesResponse {
    try {
      ...
      const services: Record<string, ActivationServiceInfo> = {};
      ...
      if (typeof data === 'object' && data !== null) {
        ...
        for (const [serviceCode, serviceData] of Object.entries(data)) {
          if (typeof serviceData === 'object' && serviceData !== null) {
            const serviceInfo = serviceData as any;
  ...
  transformServicesResponse(data: any) {
    try {
      ...
      const fullCountryServices: Record<string, any> = {
      ...
      const rentalCountries: Record<string, any> = {
   ...
   */
  transformRentResponse(data: any, originalService: string, actualCountryUsed?: string) {
    if (data && data.phone) {
      // Convert ISO country code back to numeric for database storage
      const isoToNumericMap: Record<string, string> = {
   ...
   */
  transformStatusResponse(data: any) {
    if (data && data.values) {
      const messages: any[] = [];
      
      // Transform messages to standard format
      Object.values(data.values).forEach((msg: any) => {
