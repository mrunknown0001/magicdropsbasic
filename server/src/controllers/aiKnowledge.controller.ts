 ...
 */
export const generateArticleEmbeddings = async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
export const searchKnowledgeWithEmbeddings = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized' });
    ...
    const similarities = articlesWithEmbeddings.map(item => {
      try {
        ...
        if (typeof item.embedding === 'string') {
          ...
          articleEmbedding = item.embedding as number[];
        } else {
 ...
 */
export const getAIAnalytics = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized' });
 ...
 */
export const updateAIEffectiveness = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized' });
 ...
 */
export const exportTrainingData = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized' });
    ...
    const trainingData = {
      ...
      articles: articles?.map(article => ({
        id: article.id,
        title: article.title,
        category: (article.knowledge_base_categories as any)?.name || 'Uncategorized',
 ...
 */
export const optimizeKnowledgeContext = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized' });
...
function getTopArticles(metrics: any[]): any[] {
...
function getUsageByType(metrics: any[]): any {
...
function calculatePerformanceMetrics(logs: any[]): any {
  if (logs.length === 0) {
    return {
